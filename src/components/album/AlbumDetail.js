...
import { MenuVerticalIcon, PlusCircleIcon } from "@shopify/polaris-icons";
...
const AlbumDetail = () => {
  ...
  const currentPhotos = photos
    .filter((photo) => photo.albumId === parseInt(albumId))
    .slice(indexOfFirstPhoto, indexOfLastPhoto);
  const totalPages = Math.ceil(
    photos.filter((photo) => photo.albumId === parseInt(albumId)).length /
      photosPerPage,
  );
    ...
    >
      <Layout>
        <Layout.Section>
          <Card>
            <DescriptionList
              items={[{ term: "Title", description: album.title }]}
            />
          </Card>
        </Layout.Section>
